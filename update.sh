...
jq -e '
  ...
  and .deployment.strategy == "single-compose-stable"
...
if jq -e '.services."communication-service"' "$desired_manifest" >/dev/null; then
  expected_services+=(communication-service)
fi
...
[[ ! -f "$applied_manifest" ]] && initial_install=true
...
if [[ "$initial_install" == true || "$compose_changed" == true ]]; then
  apply_command=("${compose_candidate[@]}" up -d --no-build --remove-orphans --wait --wait-timeout 120)
else
  apply_command=("${compose_candidate[@]}" up -d --no-build --no-deps --wait --wait-timeout 120 "${changed[@]}")
fi

if "${apply_command[@]}"; then
  if [[ "$compose_changed" == true ]]; then
    install -m 644 "$candidate_compose" "${install_root}/.compose.yaml.new"
